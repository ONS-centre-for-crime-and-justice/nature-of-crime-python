...
criminal_damage_seriousness_table = pd.concat([
        subtable_2,
        subtable_2_base,
        subtable_1,
        subtable_3,
...
criminal_damage_seriousness_table.reset_index(inplace=True)

criminal_damage_seriousness_table = criminal_damage_seriousness_table.drop(['index'], axis = 1)

##################################################################################
#FILLING IN MISSING DATA
#################################################################################

criminal_damage_seriousness_table.fillna(0, inplace = True)

criminal_damage_seriousness_table.replace(np.inf, np.nan, inplace = True)

criminal_damage_seriousness_table[[
        'Latest year compared with Mar 10',
        'Latest year compared with Mar 19']] = criminal_damage_seriousness_table[[
                'Latest year compared with Mar 10',
...
criminal_damage_seriousness_table_order = [0,2,1,3,4,6,5,7,8]
criminal_damage_seriousness_table = criminal_damage_seriousness_table.reindex(criminal_damage_seriousness_table_order)

criminal_damage_seriousness_table.reset_index(inplace=True)

criminal_damage_seriousness_table_6b = criminal_damage_seriousness_table.drop(['index'], axis = 1)

##################################################################################
#ADDING SUBTABLE TITLES
##################################################################################

criminal_damage_seriousness_table_6b.insert(loc=0, column='table_title', value=['' for i in range(criminal_damage_seriousness_table_6b.shape[0])])

criminal_damage_seriousness_table_6b.loc[0, 'table_title'] = 'Perception of incident'

criminal_damage_seriousness_table_6b.loc[4, 'table_title'] = 'Rated seriousness of crime'
