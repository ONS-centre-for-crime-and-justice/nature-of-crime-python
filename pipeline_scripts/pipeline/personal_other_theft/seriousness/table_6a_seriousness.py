...
subtable_1_data = personal_theft_offences[
        (personal_theft_offences['score2'] < 4)]


subtable_1 = ct.table.percentages_nonbinary(subtable_1_data, 'financial_year', 'score2', 'c11weighti', 'total_c11weighti',lookup_dict)

subtable_2_data = personal_theft_offences[
        (personal_theft_offences['crime'] < 4)]
...
personal_theft_seriousness_table =  pd.concat([
        subtable_2,
        subtable_2_base,
        subtable_1,
        subtable_3,
...
personal_theft_seriousness_table.reset_index(inplace=True)

personal_theft_seriousness_table = personal_theft_seriousness_table.drop(['index'], axis = 1)

##################################################################################
#FILLING IN MISSING DATA
#################################################################################

personal_theft_seriousness_table.fillna(0, inplace = True)

personal_theft_seriousness_table.replace(np.inf, np.nan, inplace = True)

personal_theft_seriousness_table[[
        'Latest year compared with Mar 10',
        'Latest year compared with Mar 19']] = personal_theft_seriousness_table[[
                'Latest year compared with Mar 10',
...
personal_theft_seriousness_table_order = [0,2,1,3,4,6,5,7,8]
personal_theft_seriousness_table = personal_theft_seriousness_table.reindex(personal_theft_seriousness_table_order)

personal_theft_seriousness_table.reset_index(inplace=True)
personal_theft_seriousness_table_6a = personal_theft_seriousness_table.drop(['index'], axis = 1)

##################################################################################
#ADDING SUBTABLE TITLES
##################################################################################

personal_theft_seriousness_table_6a.insert(loc=0, column='table_title', value=['' for i in range(personal_theft_seriousness_table_6a.shape[0])])

personal_theft_seriousness_table_6a.loc[0, 'table_title'] = 'Perception of incident'

personal_theft_seriousness_table_6a.loc[4, 'table_title'] = 'Rated seriousness of crime'
