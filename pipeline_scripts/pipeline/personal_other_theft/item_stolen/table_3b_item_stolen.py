...
#SELECTING THE OFFENCES FOR THE TABLE (select if offence = 67)
other_personal_theft_offences = correct_area[
        (correct_area['offence'] == 67) ]

other_personal_theft_offences.reset_index(inplace = True)
...
other_personal_theft_offences = other_personal_theft_offences[
                           (other_personal_theft_offences['whast_pt1'] == 0) |
                           (other_personal_theft_offences['whast_pt1'] == 1) ]
...
subtable_base = other_personal_theft_offences.groupby(['financial_year']).sum()
...
subtable_1 = ct.table.apply_percentages_binary(other_personal_theft_offences,'financial_year',subtable_1_variables,'c11weighti','Total_C11weighti',lookup_dict)
...
other_personal_theft_items_table = pd.concat([
        subtable_1,
        subtable_1_base,
        ], axis = 0)
other_personal_theft_items_table.reset_index(inplace = True)

##################################################################################
#FILLING IN MISSING DATA
#################################################################################

other_personal_theft_items_table.fillna(0, inplace = True)

other_personal_theft_items_table.replace(np.inf, np.nan, inplace = True)

other_personal_theft_items_table[[
        'Latest year compared with Mar 10',
        'Latest year compared with Mar 19']] = other_personal_theft_items_table[[
                'Latest year compared with Mar 10',
...
other_personal_theft_items_table_3b = other_personal_theft_items_table.drop(['index'], axis = 1)
